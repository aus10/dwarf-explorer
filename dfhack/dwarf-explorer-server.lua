...
local server = socket.tcp:bind("*", 51234)
...
while true do
  ...
  --todo: check returned straing from client to verify 
  ...
  if not err then client:send(json.encode(getUnits(), {pretty= false})) else break end
  
  sleep(4)
end

-- done with client, close the object
-- this is pointless because dfhack always crashes the second time you try to run this script 
client:close()
